...
const getNextInlineObjects = (snapshot) => {
  ...
  if (!snapshot.context.selection)
    return "";
  const endPoint = util_getTextBlockText.getSelectionEndPoint(snapshot.context.selection), block = selector_getSelectionText.getFocusBlock({
    ...snapshot,
    context: {
      ...snapshot.context,
      selection: {
  ...
  if (!block)
    return "";
  const endOfBlock = util_isEmptyTextBlock.getBlockEndPoint({
    context: snapshot.context,
    block
  });
...
exports.getBlockTextAfter = getBlockTextAfter;
