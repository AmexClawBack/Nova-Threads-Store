...
const TooltipContent$1 = (t0) => {
  ...
  }, [setPerspective, setParams, params, defaultPerspective]), schemaType = schema.get(documentType), isLiveEdit = schemaType ? isLiveEditEnabled(schemaType) : !1, isPublishedChipDisabled = useMemo(() => isLiveEdit && !selectedReleaseId ? !1 : !editState?.published, [isLiveEdit, selectedReleaseId, editState?.published]), getReleaseChipState = useCallback((release) => {
    if (!params?.historyVersion) {
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    ...
    displayed && filteredReleases.currentReleases?.map((release_0) => /* @__PURE__ */ jsx(VersionChip, { tooltipContent: /* @__PURE__ */ jsx(TooltipContent$1, { release: release_0 }), ...getReleaseChipState(release_0), onClick: handlePerspectiveChange(release_0), text: release_0.metadata.title || t("release.placeholder-untitled-release"), tone: getReleaseTone(release_0), locked: isReleaseScheduledOrScheduling(release_0), contextValues: {
      documentId: displayed?._id || "",
...
function DocumentLayout() {
  ...
  }), [currentInspector, documentIdStack, features, hasValue, inspectorMenuItems, inspectors2, previewUrl, t, stickyParams.displayInlineChanges]), handleKeyUp = useCallback((event) => {
...
const DocumentPaneProvider = memo((props) => {
  const {
    ...
    beta
  } = useWorkspace(), enhancedObjectDialogEnabled = useMemo(() => beta?.form?.enhancedObjectDialog?.enabled, [beta]), {
    selectedReleaseId,
    selectedPerspectiveName
    ...
    lastRevisionDocument
  } = useDocumentLastRev(documentId, documentType), getIsDeleted = useCallback((editState) => timelineReady ? !editState?.draft && !editState?.published && !editState?.version && !isPristine : !1, [timelineReady, isPristine]), getComparisonValue = useCallback((upstreamEditState) => {
    const upstream = selectUpstreamVersion(upstreamEditState);
    return changesOpen ? sinceDocument || upstream : upstream || null;
  }, [changesOpen, sinceDocument]), schemaType = schema.get(documentType), getIsReadOnly = useCallback((editState_0) => {
    ...
    releaseId: selectedReleaseId
  }), [documentType, documentId, actionsPerspective, selectedReleaseId]), actions = useMemo(() => documentActions2(documentActionsProps), [documentActions2, documentActionsProps]), handlePathOpen = useCallback((path) => {
  ...
  }, [onPathOpen, params, setPaneParams, enhancedObjectDialogEnabled]), badges = useMemo(() => documentBadges2({
    schemaType: documentType,
    documentId
  ...
  }, [params, setPaneParams]), handlePaneClose = useCallback(() => paneRouter.closeCurrent(), [paneRouter]), handlePaneSplit = useCallback(() => paneRouter.duplicateCurrent(), [paneRouter]), toggleInlineChanges = useCallback(() => {
  ...
  }, [router2, telemetry]), handleMenuAction = useCallback(async (item) => {
  ...
  }, [previewUrl, previousId, telemetry, buildStudioUrl, pushToast, t, documentId, documentType, handleHistoryOpen, handleInspectorAction, diffViewRouter, value_0._id, toggleInlineChanges]);
  useEffect(() => {
    setDocumentMeta({
      documentId,
      documentType,
  ...
  const compareValue = useMemo(() => getComparisonValue(upstreamEditState_0), [upstreamEditState_0, getComparisonValue]), isDeleted_1 = useMemo(() => getIsDeleted(editState_1), [editState_1, getIsDeleted]), revisionNotFound = onOlderRevision && !revisionDocument, currentDisplayed = useMemo(() => editState_1.version && isGoingToUnpublish(editState_1.version) ? editState_1.published : displayed, [editState_1.version, editState_1.published, displayed]), documentPane = useMemo(() => ({
    actions,
    activeViewId,
    badges,
    changesOpen,
  ...
  }, [formStateRef, onProgrammaticFocus, paneRouter, params, ready, enhancedObjectDialogEnabled]), /* @__PURE__ */ jsx(DocumentPaneContext.Provider, { value: documentPane, children });
});
...
const DocumentPaneProviderWrapper = memo((props) => {
  const $ = c(3), source = useSource();
  let t0;
  return $[0] !== props || $[1] !== source.beta?.eventsAPI?.documents ? (t0 = /* @__PURE__ */ jsx(DocumentPerspectiveProvider, { children: source.beta?.eventsAPI?.documents ? /* @__PURE__ */ jsx(DocumentEventsPane, { ...props }) : /* @__PURE__ */ jsx(DocumentPaneWithLegacyTimelineStore, { ...props }) }), $[0] = props, $[1] = source.beta?.eventsAPI?.documents, $[2] = t0) : t0 = $[2], t0;
});
...
function PublishedEventMenu(t0) {
  ...
  let t9;
  $[22] !== isMenuDisabled || $[23] !== t6 || $[24] !== t8 ? (t9 = /* @__PURE__ */ jsx(ContextMenuButton, { "aria-label": t6, tooltipProps: t8, disabled: isMenuDisabled }), $[22] = isMenuDisabled, $[23] = t6, $[24] = t8, $[25] = t9) : t9 = $[25];
  let t10;
  $[26] !== VersionBadge || $[27] !== event.release || $[28] !== event.versionRevisionId || $[29] !== handleOpenDraftDocument || $[30] !== handleOpenReleaseDocument || $[31] !== t || $[32] !== tCore ? (t10 = /* @__PURE__ */ jsx(Menu, { padding: 1, children: event.release ? /* @__PURE__ */ jsxs(Fragment, { children: [
  ...
  let t14;
  return $[37] !== t10 || $[38] !== t13 || $[39] !== t5 || $[40] !== t9 ? (t14 = /* @__PURE__ */ jsx(MenuButton, { id: t5, button: t9, menu: t10, popover: t13 }), $[37] = t10, $[38] = t13, $[39] = t5, $[40] = t9, $[41] = t14) : t14 = $[41], t14;
}
function _temp$1(t0) {
  const {
    children: children_2
