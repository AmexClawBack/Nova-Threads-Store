...
const TooltipContent$1 = (t0) => {
  ...
  const {
    ...
    documentType,
    documentId
  } = useDocumentPane(), isCreatingDocument = displayed && !displayed._createdAt, defaultPerspective = sanity.useGetDefaultPerspective(), filteredReleases = sanity.useFilteredReleases({
    ...
    if (sanity.isReleaseDocument(perspective) && sanity.isCardinalityOneRelease(perspective)) {
      setParams(
        {
          ...params,
          scheduledDraft: sanity.getReleaseIdFromReleaseDocumentId(perspective._id)
        },
  ...
  }, [setPerspective, setParams, params, defaultPerspective]), schemaType = schema.get(documentType), isLiveEdit = schemaType ? isLiveEditEnabled(schemaType) : !1, isPublishedChipDisabled = React.useMemo(() => isLiveEdit && !selectedReleaseId ? !1 : !editState?.published, [isLiveEdit, selectedReleaseId, editState?.published]), getReleaseChipState = React.useCallback((release) => {
    if (!params?.historyVersion) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    ...
    displayed && filteredReleases.currentReleases?.map((release_0) => /* @__PURE__ */ jsxRuntime.jsx(sanity.VersionChip, { tooltipContent: /* @__PURE__ */ jsxRuntime.jsx(TooltipContent$1, { release: release_0 }), ...getReleaseChipState(release_0), onClick: handlePerspectiveChange(release_0), text: release_0.metadata.title || t("release.placeholder-untitled-release"), tone: sanity.getReleaseTone(release_0), locked: sanity.isReleaseScheduledOrScheduling(release_0), contextValues: {
      documentId: displayed?._id || "",
...
function DocumentLayout() {
  ...
  }), [currentInspector, documentIdStack, features, hasValue, inspectorMenuItems, inspectors2, previewUrl, t, stickyParams.displayInlineChanges]), handleKeyUp = React.useCallback((event) => {
...
const DocumentPaneProvider = React.memo((props) => {
  const {
  ...
  } = sanity.useSource(), telemetry2 = react.useTelemetry(), router2 = router$1.useRouter(), paneRouter = usePaneRouter(), setPaneParams = paneRouter.setParams, {
    push: pushToast
  } = ui.useToast(), {
    options,
    ...
    beta
  } = sanity.useWorkspace(), enhancedObjectDialogEnabled = React.useMemo(() => beta?.form?.enhancedObjectDialog?.enabled, [beta]), {
    selectedReleaseId,
    selectedPerspectiveName
    ...
    lastRevisionDocument
  } = useDocumentLastRev(documentId, documentType), getIsDeleted = React.useCallback((editState) => timelineReady ? !editState?.draft && !editState?.published && !editState?.version && !isPristine : !1, [timelineReady, isPristine]), getComparisonValue = React.useCallback((upstreamEditState) => {
    const upstream = sanity.selectUpstreamVersion(upstreamEditState);
    return changesOpen ? sinceDocument || upstream : upstream || null;
  }, [changesOpen, sinceDocument]), schemaType = schema.get(documentType), getIsReadOnly = React.useCallback((editState_0) => {
    ...
    releaseId: selectedReleaseId
  }), [documentType, documentId, actionsPerspective, selectedReleaseId]), actions = React.useMemo(() => documentActions2(documentActionsProps), [documentActions2, documentActionsProps]), handlePathOpen = React.useCallback((path) => {
  ...
  }, [onPathOpen, params, setPaneParams, enhancedObjectDialogEnabled]), badges = React.useMemo(() => documentBadges2({
    schemaType: documentType,
    documentId
  ...
  }, [params, setPaneParams]), handlePaneClose = React.useCallback(() => paneRouter.closeCurrent(), [paneRouter]), handlePaneSplit = React.useCallback(() => paneRouter.duplicateCurrent(), [paneRouter]), toggleInlineChanges = React.useCallback(() => {
  ...
  }, [router2, telemetry2]), handleMenuAction = React.useCallback(async (item) => {
  ...
  }, [previewUrl, previousId, telemetry2, buildStudioUrl, pushToast, t, documentId, documentType, handleHistoryOpen, handleInspectorAction, diffViewRouter, value_0._id, toggleInlineChanges]);
  React.useEffect(() => {
    setDocumentMeta({
      documentId,
      documentType,
  ...
  const compareValue = React.useMemo(() => getComparisonValue(upstreamEditState_0), [upstreamEditState_0, getComparisonValue]), isDeleted_1 = React.useMemo(() => getIsDeleted(editState_1), [editState_1, getIsDeleted]), revisionNotFound = onOlderRevision && !revisionDocument, currentDisplayed = React.useMemo(() => editState_1.version && sanity.isGoingToUnpublish(editState_1.version) ? editState_1.published : displayed, [editState_1.version, editState_1.published, displayed]), documentPane = React.useMemo(() => ({
    actions,
    activeViewId,
    badges,
    changesOpen,
  ...
  }, [formStateRef, onProgrammaticFocus, paneRouter, params, ready, enhancedObjectDialogEnabled]), /* @__PURE__ */ jsxRuntime.jsx(_singletons.DocumentPaneContext.Provider, { value: documentPane, children });
});
...
const DocumentPaneProviderWrapper = React.memo((props) => {
  const $ = reactCompilerRuntime.c(3), source = sanity.useSource();
  let t0;
  return $[0] !== props || $[1] !== source.beta?.eventsAPI?.documents ? (t0 = /* @__PURE__ */ jsxRuntime.jsx(DocumentPerspectiveProvider, { children: source.beta?.eventsAPI?.documents ? /* @__PURE__ */ jsxRuntime.jsx(DocumentEventsPane, { ...props }) : /* @__PURE__ */ jsxRuntime.jsx(DocumentPaneWithLegacyTimelineStore, { ...props }) }), $[0] = props, $[1] = source.beta?.eventsAPI?.documents, $[2] = t0) : t0 = $[2], t0;
});
...
function PublishedEventMenu(t0) {
  ...
  const VersionBadge = t4, isMenuDisabled = event.release && !sanity.isReleaseDocument(event.release), t5 = `timeline-item-menu-button-${event.versionId}`;
  let t6;
  $[14] !== t ? (t6 = t("timeline-item.menu-button.aria-label"), $[14] = t, $[15] = t6) : t6 = $[15];
  let t7;
  $[16] !== event.release || $[17] !== isMenuDisabled || $[18] !== t ? (t7 = isMenuDisabled ? t("timeline-item.not-found-release.tooltip", {
    releaseId: event.release?._id ? sanity.getReleaseIdFromReleaseDocumentId(event.release._id) : void 0
  }) : t("timeline-item.menu-button.tooltip"), $[16] = event.release, $[17] = isMenuDisabled, $[18] = t, $[19] = t7) : t7 = $[19];
  let t8;
  $[20] !== t7 ? (t8 = {
    content: t7
  }, $[20] = t7, $[21] = t8) : t8 = $[21];
  let t9;
  $[22] !== isMenuDisabled || $[23] !== t6 || $[24] !== t8 ? (t9 = /* @__PURE__ */ jsxRuntime.jsx(sanity.ContextMenuButton, { "aria-label": t6, tooltipProps: t8, disabled: isMenuDisabled }), $[22] = isMenuDisabled, $[23] = t6, $[24] = t8, $[25] = t9) : t9 = $[25];
  let t10;
  $[26] !== VersionBadge || $[27] !== event.release || $[28] !== event.versionRevisionId || $[29] !== handleOpenDraftDocument || $[30] !== handleOpenReleaseDocument || $[31] !== t || $[32] !== tCore ? (t10 = /* @__PURE__ */ jsxRuntime.jsx(ui.Menu, { padding: 1, children: event.release ? /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
  ...
  }, i18nKey: "events.open.draft", t }) }) }) }) }), $[26] = VersionBadge, $[27] = event.release, $[28] = event.versionRevisionId, $[29] = handleOpenDraftDocument, $[30] = handleOpenReleaseDocument, $[31] = t, $[32] = tCore, $[33] = t10) : t10 = $[33];
  const t11 = portalContext.elements?.[TIMELINE_MENU_PORTAL$1] ? TIMELINE_MENU_PORTAL$1 : !0;
  let t12;
  ...
  let t14;
  return $[37] !== t10 || $[38] !== t13 || $[39] !== t5 || $[40] !== t9 ? (t14 = /* @__PURE__ */ jsxRuntime.jsx(TooltipDelayGroupProvider.MenuButton, { id: t5, button: t9, menu: t10, popover: t13 }), $[37] = t10, $[38] = t13, $[39] = t5, $[40] = t9, $[41] = t14) : t14 = $[41], t14;
}
function _temp$1(t0) {
  const {
    children: children_2
